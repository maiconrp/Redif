...
def perfilUsuario(request, user):
    ...
    RedacoesAvaliadas = perfil[0].Usuario_avaliacao.all()
    context = {
        'Redacao': list(Redacoes),
        'RedacoesAvaliadas': RedacoesAvaliadas,
        'Perfil' : perfil[0],
        'Usuario' :  usuario(request),
        'Titulacao' : str(titulacao[perfil[0].titulacao]),
        'Condicao' : str(condicao[perfil[0].tipo]),
        'Username' : str(perfil[0].username).upper(),
        'qtdAvaliadas': perfil[0].Usuario_avaliacao.all().count(),
...
def PagDicas(request, iDica):
    ...
    if iDica in Dicas.keys():
        return render(request, 'PaginaDeDicas/' + Dicas[iDica] + '.html', {'Redacoes' : Redacoes, 'Usuario' : usuario(request)})
    else: 
        return render(request, 'PaginaDeDicas/PagDicasGeral.html', {'Redacoes' : Redacoes, 'Usuario' : usuario(request) })
